...
export default function VehicleInventory() {
  ...
  const SUCURSALES = [
    ...
    ...[...new Set(vehiculosData.map((v) => v.sucursal))].map((suc) => ({
      value: suc,
      label: suc,
    })),
  ];
  const TIPOS = [
    { value: "Todos", label: t("VehicleInventary.all_m") }, // "Todos"
    ...[...new Set(vehiculosData.map((v) => v.tipo))].map((tipo) => ({
      value: tipo,
      label: tipo,
    })),
  ];
  ...
  // Filtrado lógico
  const filtered = vehiculosData.filter(
    (v) =>
      (search === "" ||
      ...
      (selectedFilters.sucursal === null ||
        ...
        v.sucursal === selectedFilters.sucursal) &&
      (selectedFilters.tipo === null ||
        selectedFilters.tipo === "Todos" ||
        v.tipo === selectedFilters.tipo),
  );
  ...
  const stats = {
    ...
    mantenimiento: vehiculosData.filter((v) => v.estado === "Mantenimiento")
      .length,
  };
      ...
      <div className="vi-wrapper">
        ...
        <div className="vi-stats">
          ...
          <div className="vi-stat disponible">
            ...
            <span className="vi-stat-label">
              {t("VehicleInventary.available")}
            </span>
          </div>
          ...
          <div className="vi-stat mantenimiento">
            ...
            <span className="vi-stat-label">
              {t("VehicleInventary.maintenance")}
            </span>
          </div>
        </div>
        ...
        <div className="vi-controls">
          ...
          <div className="vi-filters" ref={filtersRef}>
            {[
              {
                key: "estado",
                label: t("VehicleInventary.keyState"),
                options: ESTADOS,
              },
              {
                key: "sucursal",
                label: t("VehicleInventary.keyBranch"),
                options: SUCURSALES,
              },
              {
                key: "tipo",
                label: t("VehicleInventary.keyType"),
                options: TIPOS,
              },
                  ...
                  <div className="vi-dropdown">
                    {ESTADOS.map((opcion) => (
                      <button
                        ...
                        onClick={() =>
                          handleFilterSelect("estado", opcion.value)
                        }
                      >
                        {opcion.label}{" "}
        ...
        {vista === "grid" && (
          <div className="vi-grid">
            {filtered.map((v) => (
              <div
                key={v.id}
                className={`vi-card estado-${v.estado.replace(" ", "-").toLowerCase()}`}
              ...
              >
                ...
                <div className="vi-card-body">
                  <div className="vi-card-top">
                    <span
                      className={`vi-badge ${v.estado.replace(" ", "-").toLowerCase()}`}
                    >
                      {v.estado === "Disponible" &&
                        t("VehicleInventary.available")}
            ...
            {filtered.length === 0 && (
              <div className="vi-empty">{t("VehicleInventary.notFound")}</div>
            )}
          </div>
        )}
          ...
          <div className="vi-table-wrap">
            <table className="vi-table">
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan="8" className="vi-empty">
                      {t("VehicleInventary.notFound")}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="vi-modal-overlay" onClick={() => setSelected(null)}>
          <div className="vi-modal" onClick={(e) => e.stopPropagation()}>
            <div className="vi-modal-header">
              <div>
                <p className="vi-modal-label">
                  {t("VehicleInventary.vehicleDetails")}
                </p>
                <h2 className="vi-modal-title">
                  {selected.marca} {selected.modelo}
                </h2>
              </div>
              <span
                className={`vi-badge ${selected.estado.replace(" ", "-").toLowerCase()}`}
              >
            ...
            <div className="vi-modal-grid">
              <div className="vi-modal-field">
                <span className="vi-modal-field-label">
                  {t("VehicleInventary.plate")}
                </span>
                <span className="vi-modal-field-value">{selected.placa}</span>
              </div>
              <div className="vi-modal-field">
                <span className="vi-modal-field-label">
                  {t("VehicleInventary.year")}
                </span>
                <span className="vi-modal-field-value">{selected.año}</span>
              </div>
              <div className="vi-modal-field">
                <span className="vi-modal-field-label">
                  {t("VehicleInventary.type")}
                </span>
                <span className="vi-modal-field-value">{selected.tipo}</span>
              </div>
              <div className="vi-modal-field">
                <span className="vi-modal-field-label">
                  {t("VehicleInventary.mileage")}
                </span>
                <span className="vi-modal-field-value">
                  {selected.km.toLocaleString()} km
                </span>
              </div>
              <div className="vi-modal-field" style={{ gridColumn: "1 / -1" }}>
                <span className="vi-modal-field-label">
                  {t("VehicleInventary.branch")}
                </span>
                <span className="vi-modal-field-value">
                  {selected.sucursal}
                </span>
            ...
            <div className="vi-modal-footer">
              <button
                className="vi-modal-btn-secondary"
                onClick={() => setSelected(null)}
              >
                {t("VehicleInventary.close")}
              </button>
              <button
                className="vi-modal-btn-primary"
                onClick={() => navigate("/CheckStatus")}
              >
                {t("VehicleInventary.stateSee")}
              </button>
            </div>
          </div>
        </div>
