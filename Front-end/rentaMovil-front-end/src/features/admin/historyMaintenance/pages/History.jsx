...
function History() {
  ...
  const handleSaveEdit = (data) => {
    const updated = records.map(r => r.id === selected.id ? { ...r, ...data } : r);
    setRecords(updated);
    setSelected({ ...selected, ...data });// de estama manera se estaria sobre escribiendo los valores viejos por los nuevos
    setIsEditing(false);
  }
